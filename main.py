...
parser.add_argument(
    "--target-price", default=None, type=float, help="Your target price for the product"
)
...
if price and price <= args.target_price:
    print(
        f"Horray! current price:{price}, is less than or equal to your target price: {args.target_price}"
    )
else:
    print(
        f"Sorry! current price: {price}, is still higher than your target price: {args.target_price}"
    )
